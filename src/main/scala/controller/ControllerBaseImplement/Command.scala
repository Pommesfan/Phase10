...
class CreatePlayerCommand(players:List[String], oldState: ControllerStateInterface) extends Command:
  override def doStep(c:ControllerInterface):(ControllerStateInterface, OutputEvent) = oldState.asInstanceOf[InitialState].createPlayers(players, c)
...
class DiscardCommand(indices: List[List[Int]], oldState: ControllerStateInterface) extends Command:
  override def doStep(c:ControllerInterface):(ControllerStateInterface, OutputEvent) =   oldState.asInstanceOf[DiscardControllerState].discardCards(indices, c)
...
class NoDiscardCommand(oldState: ControllerStateInterface) extends Command:
  override def doStep(c:ControllerInterface):(ControllerStateInterface, OutputEvent) =   oldState.asInstanceOf[DiscardControllerState].skipDiscard(c)
  override def undoStep(c:ControllerInterface):(ControllerStateInterface, OutputEvent) = (oldState, new GoToDiscardEvent)
...
class InjectCommand(receiving_player:Int, cardIndex:Int, stashIndex:Int, position:Int, oldState: ControllerStateInterface) extends Command:
  override def doStep(c:ControllerInterface):(ControllerStateInterface, OutputEvent) =   oldState.asInstanceOf[InjectControllerState].injectCard(receiving_player, cardIndex, stashIndex, position, c)
...
class NoInjectCommand(oldState: ControllerStateInterface) extends Command:
  override def doStep(c:ControllerInterface):(ControllerStateInterface, OutputEvent) =   oldState.asInstanceOf[InjectControllerState].skipInject(c)
  override def undoStep(c:ControllerInterface):(ControllerStateInterface, OutputEvent) = (oldState, new GoToInjectEvent)