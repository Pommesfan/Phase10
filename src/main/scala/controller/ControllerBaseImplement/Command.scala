...
class CreatePlayerCommand(players:List[String], oldState: ControllerStateInterface) extends Command[Controller]:
...
class SwitchCardCommand(index:Int, mode:Int, oldState: ControllerStateInterface) extends Command[Controller]:
...
class DiscardCommand(indices: List[List[Int]], oldState: ControllerStateInterface) extends Command[Controller]:
...
class NoDiscardCommand(oldState: ControllerStateInterface) extends Command[Controller]:
...
class InjectCommand(receiving_player:Int, cardIndex:Int, stashIndex:Int, position:Int, oldState: ControllerStateInterface) extends Command[Controller]:
...
class NoInjectCommand(oldState: ControllerStateInterface) extends Command[Controller]:
